...
#define SPI_RAM_BUFFER_SIZE                                                    \
  1164U /* Size of SPI buffer used to store received data from the host */
...
static uint8_t
OPENBL_SPI_GetSpecialCmdOpCode(uint16_t *OpCode,
                               OPENBL_SpecialCmdTypeTypeDef CmdType);
static uint8_t
OPENBL_SPI_ConstructCommandsTable(OPENBL_CommandsTypeDef *pSpiCmd);
...
/**
 * @brief  This function is used to set a pointer to the structure that contains
 * the available SPI commands.
 * @return Returns a pointer to the OPENBL_SPI_Commands struct.
 */
 ...
 *         so the send of the second acknowledge after disabling the read
 * protection is not possible, this is why the two ACKs are sent successively
 * @retval None.
 */
    ...
    if ((data & 0xFFF0U) == 0xFFF0U) {
      ...
      if (OPENBL_SPI_ReadByte() != (uint8_t) xor) {
        ...
        if ((data == 0xFFFFU) || (data == 0xFFFEU) || (data == 0xFFFDU)) {
          ...
          error_value = OPENBL_MEM_MassErase(
              OPENBL_DEFAULT_MEM, (uint8_t *)SPI_RAM_Buf, SPI_RAM_BUFFER_SIZE);
...
void OPENBL_SPI_SpecialCommand(void) {
  ...
  if (OPENBL_SPI_GetSpecialCmdOpCode(&op_code, OPENBL_SPECIAL_CMD) ==
      NACK_BYTE) {
    OPENBL_SPI_SendAcknowledgeByte(NACK_BYTE);
  } else {
...
void OPENBL_SPI_ExtendedSpecialCommand(void) {
  ...
  /* Get the command operation code */
  if (OPENBL_SPI_GetSpecialCmdOpCode(&op_code, OPENBL_EXTENDED_SPECIAL_CMD) ==
      NACK_BYTE) {
    OPENBL_SPI_SendAcknowledgeByte(NACK_BYTE);
  } else {
    /* Send Operation code acknowledgment */
