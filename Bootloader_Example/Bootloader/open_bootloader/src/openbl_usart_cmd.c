...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define OPENBL_USART_COMMANDS_NB_MAX                                           \
  13U /* The maximum number of supported commands */

#define USART_RAM_BUFFER_SIZE                                                  \
  1164U /* Size of USART buffer used to store received data from the host */
...
/* Private variables ---------------------------------------------------------*/
static uint8_t
    USART_RAM_Buf[USART_RAM_BUFFER_SIZE]; /* Buffer used to store received data
                                             from the host */
...
static uint8_t
OPENBL_USART_GetSpecialCmdOpCode(uint16_t *OpCode,
                                 OPENBL_SpecialCmdTypeTypeDef CmdType);
static uint8_t
OPENBL_USART_ConstructCommandsTable(OPENBL_CommandsTypeDef *pUsartCmd);

/* Exported variables --------------------------------------------------------*/
/* Exported functions---------------------------------------------------------*/

/**
 * @brief  This function is used to get a pointer to the structure that contains
 * the available USART commands.
...
void OPENBL_USART_WriteMemory(void) {
  ...
  if (Common_GetProtectionStatus() != RESET) {
    ...
    if (OPENBL_USART_GetAddress(&address) == NACK_BYTE) {
      ...
      /* Read the number of bytes to be written: Max number of data = data + 1 =
       * 256 */
      data = OPENBL_USART_ReadByte();
...
void OPENBL_USART_ReadoutUnprotect(void) {
  ...
  /* Once the option bytes modification start bit is set in FLASH CR register,
     ...
     This is why the last ACK is sent before the call of
     OPENBL_MEM_SetReadOutProtection */
  OPENBL_USART_SendByte(ACK_BYTE);
    ...
    if ((data & 0xFFF0U) == 0xFFF0U) {
      ...
      if (OPENBL_USART_ReadByte() != (uint8_t) xor) {
        ...
        if ((data == 0xFFFFU) || (data == 0xFFFEU) || (data == 0xFFFDU)) {
          ...
          error_value =
              OPENBL_MEM_MassErase(OPENBL_DEFAULT_MEM, (uint8_t *)USART_RAM_Buf,
      ...
      *ramaddress = (uint8_t)((numpage & 0xFF00U) >> 8);
      ...
      if (OPENBL_USART_ReadByte() != (uint8_t) xor) {
        ...
        error_value =
            OPENBL_MEM_Erase(OPENBL_DEFAULT_MEM, (uint8_t *)USART_RAM_Buf,
                             USART_RAM_BUFFER_SIZE);
...
void OPENBL_USART_WriteProtect(void) {
  ...
  if (Common_GetProtectionStatus() != RESET) {
    ...
    if (OPENBL_USART_ReadByte() != (uint8_t) xor) {
      ...
      error_value = OPENBL_MEM_SetWriteProtection(ENABLE, OPENBL_DEFAULT_MEM,
                                                  ramaddress, length);
...
void OPENBL_USART_WriteUnprotect(void) {
  ...
  if (Common_GetProtectionStatus() != RESET) {
    ...
    error_value =
        OPENBL_MEM_SetWriteProtection(DISABLE, OPENBL_DEFAULT_MEM, NULL, 0);
...
uint8_t OPENBL_USART_GetAddress(uint32_t *Address) {
  ...
  if (OPENBL_USART_ReadByte() != xor) {
    ...
    *Address = ((uint32_t)data[3] << 24) | ((uint32_t)data[2] << 16) |
...
void OPENBL_USART_SpecialCommand(void) {
  ...
  if (OPENBL_USART_GetSpecialCmdOpCode(&op_code, OPENBL_SPECIAL_CMD) ==
      NACK_BYTE) {
    OPENBL_USART_SendByte(NACK_BYTE);
  } else {
...
void OPENBL_USART_ExtendedSpecialCommand(void) {
  ...
  /* Get the command operation code */
  if (OPENBL_USART_GetSpecialCmdOpCode(&op_code, OPENBL_EXTENDED_SPECIAL_CMD) ==
      NACK_BYTE) {
    OPENBL_USART_SendByte(NACK_BYTE);
  } else {
    /* Send Operation code acknowledgment */
 ...
 * @param  CmdType Type of the command, Special command or extended special
 * command.
 * @retval Returns NACK status in case of error else returns ACK status.
 */
static uint8_t
